	...
	*/
	...
	function servicio__cascadas_efs()
	{
		if (! isset($_GET['cascadas-ef']) || ! isset($_GET['cascadas-maestros'])) {
			throw new toba_error_seguridad("Cascadas: Invocaci�n incorrecta");	
		}
	...
	/**
	 * M�todo que se utiliza en la respuesta del filtro del combo editable usando AJAX
	 */
	function servicio__filtrado_ef_ce()
	{
		if (! isset($_GET['filtrado-ce-ef']) || ! isset($_GET['filtrado-ce-valor'])) {
			throw new toba_error_seguridad("Filtrado de combo editable: Invocaci�n incorrecta");	
		}
	...
	/**
	 * M�todo que se utiliza en la respuesta del filtro del combo editable cuando se quiere validar un id seleccionado
	 */
	function servicio__filtrado_ef_ce_validar()
	{
		if (! isset($_GET['filtrado-ce-ef']) || ! isset($_GET['filtrado-ce-valor'])) {
			throw new toba_error_seguridad("Validaci�n de combo editable: Invocaci�n incorrecta");	
		}
		$id_ef = trim(toba::memoria()->get_parametro('filtrado-ce-ef'));
	...
	function generar_html()
	{
		...
		toba::output()->get('Formulario')->getInicioHtml($this->_estilos, $ancho);
		...
		toba::output()->get('Formulario')->getFinHtml();
		$this->_flag_out = true;
	}
	...
	protected function generar_formulario()
	{
		...
		toba::output()->get('Formulario')->getInicioFormulario("cuerpo_{$this->objeto_js}",$ancho,$colapsado);
		$this->generar_layout();
		...
		if ($hay_colapsado) {
			toba::output()->get('Formulario')->getColapsadoTrigger("{$this->objeto_js}_cambiar_expansion","{$this->objeto_js}.cambiar_expansion()");
		}
		if ($this->botonera_abajo()) {
			$this->generar_botones();
		}
		toba::output()->get('Formulario')->getFinFormulario();
	}
	...
	protected function generar_layout_template_impresion()
	{
		...
		if (preg_match_all($pattern, $this->_info_formulario['template_impresion'], $resultado)) {
			...
			throw new toba_error_def($this->get_txt()."Template impresi�n incorrecto");
		}
	}
	...
	{
		$this->get_html_ef($ef, $ancho_etiqueta);
	}
	...
	protected function crear_objeto_js()
	{
		...
		foreach ($this->_lista_ef_post as $ef) {
			if (! in_array($ef, $this->_efs_generados)) {
				throw new toba_error_def($this->get_txt()." Error en la redefinici�n del layout: Falta salida ef '$ef'");
			}
