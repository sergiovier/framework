<?php
/**
 * Una pantalla es la parte gr�fica de una etapa del controlador de interface (ci).
...
class toba_ei_pantalla extends toba_ei
{
	...
	protected function aplicar_restricciones_funcionales()
	{
		...
		//-- Restricci�n funcional pantalla no-visible ------		
		...
		//-- Restricci�n funcional eis no-visible ------
	...
	/**
	 * Retorna la descripci�n de esta pantalla
	 * @return string
	 */
	...
	/**
	 * Cambia la descripci�n de esta pantalla
	 * @param string $descr
	...
	/**
	 * Cambia el tipo de navegaci�n de la pantalla
	 *
	 ...
	 * La dependencia tiene que estar asignada al ci actual, este m�todo s�lo indica que esta dependencia
	 * se graficar� en esta pantalla
	...
	function agregar_dep($id_obj)
	{
		...
		if ($this->controlador->existe_dependencia($id_obj)) {
			...
			toba::logger()->error($this->get_txt(). 
					" Se quiere agregar la dependencia '$id_obj', pero esta no est� definida en el CI");
		}
	...
	/**
	 * Determina que una dependencia no ser� mostrada en la pantalla actual
	...
	function eliminar_dep($id)
	{
		if (in_array($id, $this->_lista_dependencias)) {
			...
			throw new toba_error_def($this->get_txt(). 
					" Se quiere eliminar la dependencia '$id', pero esta no est� en la pantalla actual");
		}
		//--- Por si ya estamos en la etapa de servicios
	...
	/**
	 * Acceso a un tab o solapa espec�fico
	 ...
	 * La consecuencia es que ya no es posible accederlo m�s durante el pedido de p�gina actual, 
	 * y al momento de graficar la barra de tabs, no ser� incluido
	...
	function eliminar_tab($id)
	{
		...
		if (isset($this->_lista_tabs[$id])) {
			...
			throw new toba_error_def($this->get_txt(). 
					" Se quiere eliminar el tab '$id', pero esta no est� en la pantalla actual");
		}
	}
	...
	/**
	 * Carga la lista de botones que representan a las pesta�as o tabs que se muestran en la pantalla actual
	 * @ignore 
	 */
	 ...
	 * Esto es necesario explitarlo cuando la forma de navegaci�n no es a trav�s de tabs
	...
	/**
	 * Grafica el contenido de la pantalla actual, por defecto incluye una secci�n de descripci�n
	 * @ignore 
	 */
	...
	/**
	 * Dispara la generaci�n de html de los objetos contenidos en esta pantalla
	...
	function generar_layout_template_impresion(toba_impresion $obj_salida)
	{
		...
		if (! empty($restantes)) {
			$faltan = implode(', ', $restantes);
			throw new toba_error_def($this->get_txt(). " Template de impresi�n incompleto, falta incluir las siguientes dependencias: $faltan");
		}
	}
