...
class toba_tab extends toba_boton
{
	...
	function get_html($tipo, $id_submit, $id_componente, $seleccionado, $editor='')
	{
		...
		$id = $escapador->escapeHtmlAttr($id_submit.'_cambiar_tab_'.$evento);
		$js = "onclick=\"". $escapador->escapeHtmlAttr($id_componente).".ir_a_pantalla('". $escapador->escapeHtmlAttr($evento)."');return false;\"";
		$js_extra = '';
		if ( $this->activado ) {
			$clase_boton = '';			
		} else {
